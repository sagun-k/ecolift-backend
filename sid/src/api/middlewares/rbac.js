...
export function authorize() {
  return (req, res, next) => {
    console.log(`Checking permissions for ${req.user?.role}: ${req.method} - ${req.originalUrl}`);
    ...
    if (!resource) {
      return res.status(400).json({ error: 'Bad Request: Unable to determine resource' });
    }

    // Map HTTP methods to permission strings dynamically
    const methodPermission = `${resource}:${req.method.toLowerCase()}`;

    if (allowedPermissions.includes('*') || allowedPermissions.includes(methodPermission)) {
      return next();
    }
